                ...
                <tbody>
                    {users.map(user => (
                        <tr key={user._id}>
                            ...
                            <td data-label="Arrival date" style={{ width: "100px" }}>{formatDate(user.arrival_date) || 'N/A'}</td>
                            <td data-label="Arrival date" style={{ width: "100px" }}>{user.last_interaction_format || 'N/A'}</td>
                            ...
                            <td data-label="First Question">{user.first_question === 'want_to_participate_in_events' ? 'Хочу участвовать в событиях' : user.first_question === 'want_to_be_rehabilitologist' ? 'Хочу быть реабилитологом': user.first_question === 'want_to_upgrade_qualification' ? 'Хочу повысить квалификацию':''}</td>
                            <td data-label="Second Question">{user.second_question || 'N/A'}</td>
                            <td data-label="Payment Status Module 1" className={user.payment_status_module_1 === 'Paid' ? 'Оплачено' : 'not-paid'}>
                            ...
                            <td data-label="Payment Status Module 3" className={user.payment_status_module_3 === 'Paid' ? 'Оплачено' : 'not-paid'}>
                                {user.payment_status_module_3 === 'Paid' ? 'Оплачено' : 'Не оплачено'}
                            </td>
                            <td data-label="Payment Status All Course" className={user.payment_status_all_course === 'Paid' ? 'Оплачено' : 'not-paid'}>
